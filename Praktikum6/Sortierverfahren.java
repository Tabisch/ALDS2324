public class Sortierverfahren
{
	 ...
	 public static void insertionSort(Comparable[] data)
	 {
	    ...
	    {
	       // Entnehmen des einzuf�genden Elements
	       Comparable temp = data[i];
	       // Bestimmen der Position k, hinter der eingef�gt wird
	       int k = i-1;
	       ...
	       // Einf�gen des neuen Elements an hinter Position k
	       data[k+1] = temp;
	    }
	 }
	 ...
	 public static void binaryInsertionSort(Comparable[] data)
	 {
	    for (int i=1; i < data.length; i++)
	    {
	       // Bestimmen der Einf�geposition k im Bereich 0..i-1
	       int k = 0, l = i-1;
	       while (k <= l)
	       {
	    	   int m = (k + l)/2;
	       ...
	       for (int j = i; j > k; j--)
	          data[j] = data[j-1];
	       // Einf�gen des neuen Elements an Position k
	       data[k] = temp;
	    }
	 }
	 ...
	 { 
		 ...
		 while (k+1 <= right/2) // data[k+1] hat linken Sohn
		 {
			 ...
			 if (x.compareTo(data[j]) >= 0)
				 break; // Halte an, Heap-Bedingung erf�llt!
			 else
			 {
				 data[k] = data[j];
				 k = j;
